local function getVehicles(cid)
    local result = MySQL.query.await(
        'SELECT vehicle, plate, fuel, engine, body FROM player_vehicles WHERE citizenid = ?', 
        { cid }
    )
    local vehicles = {}
...
RegisterNetEvent('ps-adminmenu:server:SetJob', function(data, selectedData)
    ...
    local Job = selectedData["Job"].value
    local Grade = selectedData["Grade"].value
    ...
    if not jobInfo then
        TriggerClientEvent('QBCore:Notify', src, "Not a valid job", 'error')
        return
    end

    local gradeIndex = tostring(Grade)
    local grade = jobInfo.grades[gradeIndex]

    if not grade then
        TriggerClientEvent('QBCore:Notify', src, "Not a valid grade", 'error')
        return
    end
...
RegisterNetEvent('ps-adminmenu:server:SetGang', function(data, selectedData)
    ...
    local src = source
    local playerId = selectedData["Player"].value
    local Gang = selectedData["Gang"].value
    local Grade = selectedData["Grade"].value
    ...
    if not GangInfo then
        TriggerClientEvent('QBCore:Notify', src, "Not a valid Gang", 'error')
        return
    end

    local gradeIndex = tostring(Grade)
    local grade = GangInfo.grades[gradeIndex]
